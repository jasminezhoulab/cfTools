...
print(markerParam)
```
Note that errors may occur because of parameter value `NA:NA` or `0:0`. Remove these values beforehand.
...
markers. Additionally, there are a few more columns which can be derived 
from the output of `MergeCpGs()`.

```{r}
fragMethInfo <- file.path(demo.dir, "cfSort.reads.txt.gz")
head(read.csv(fragMethInfo, header=TRUE, sep="\t", 
              colClasses = "character"), 4)
```
Column *methCount* is the number of methylated CpG sites (methState=1) 
on the fragment, and column *unmethCount* is the number of unmethylated 
CpG sites (methState=0) on the fragment.

The output is a list containing the cfDNA fractions of 29 tissue types.

```{r, message=FALSE}
tissueFraction2 <- cfSort(fragMethInfo, "demo")
tissueFraction2
```

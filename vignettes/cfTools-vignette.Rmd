...
abstract: >
  ...
  studies, including (1) cancer detection: sensitively detect 
  ...
  composition and the cfDNA fraction of multiple tissue types 
  for a plasma cfDNA sample.
vignette: >
...
2. Tissue deconvolution [2]: separate cfDNA fragments from different tissues 
...
* Section 3.3 `GenerateFragMeth()`
  * Input: the output lists of `CollapsePEReads()` and `CollapseCpGs()`
...
Function `CollapsePEReads()` generates fragment-level information for 
...
Function `CollapseCpGs()` generates fragment-level methylation states of CpGs. 
...
`CollapsePEReads()` and `CollapseCpGs()` into one list, 
...
frag_bed <- read.csv(file.path(demo.dir, "demo.fragment_level.meth.bed.txt"), 
                     header=TRUE, sep="\t")
...
markers_bed <- read.csv(file.path(demo.dir, "markers.bed.txt"), 
                        header=TRUE, sep="\t")

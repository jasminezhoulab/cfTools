...
#' @param CpG_OB a file of methylation information for CpG on the original bottom strand (OB), 
#' which is one of the outputs from `bismark methylation extractor`.
...
CollapseCpGs <- function(CpG_OT, CpG_OB, output.dir="", id="", python="python") {
  ...
  output_bed <- read.csv(refo_meth, sep="\t", header = FALSE, colClasses = "character")
  ...
  if (output.dir=="" | id=="") {
    system2(command = "rm", args = refo_meth)
  }

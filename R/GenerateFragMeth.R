#' @title
#' Generate fragment level methylation status
#'
#' @description
#' Join two data frames containing read information and methylation status on each read into one data frame.
#'
#' @param frag_bed a BED file containing information for every read, which is the output of CollapsePEReads().
#' @param meth_bed a BED file containing methylation status on every read, which is the output of CollapseCpGs().
...
GenerateFragMeth <- function(frag_bed, meth_bed, output.dir="", id="", python="python") {
  ...
  output_bed <- as.data.frame(cbind(merged_file$chr.x, merged_file$start, merged_file$end, merged_file$name, 
                ...
                merged_file$cpgPosition, merged_file$methStatus))
  colnames(output_bed) <- c("chr", "start", "end", "name", "fragmentLength", "strand", 
                            "cpgNumber", "cpgPosition", "methStatus")
